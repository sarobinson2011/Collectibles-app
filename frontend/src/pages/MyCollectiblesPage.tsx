...
import { fetchListings, type Collectible, type Listing } from "../api";
import { useWallet } from "../eth/wallet";
import { NFT_ADDRESS } from "../eth/config";

type ViewMode = "grid" | "list";
type SortKey = "tokenId" | "rfid";
...
export function MyCollectiblesPage() {
    ...
    return (
        <div
            style={{
                // prevent “giant cards” on wide screens (same as AllCollectiblesPage)
                maxWidth: "1200px",
                margin: "0 auto",
                padding: "0 1rem",
            }}
        >
            {/* Grid styling with breakpoints (same formatting as AllCollectiblesPage) */}
            <style>{`
                .collectibles-grid {
                    margin-top: 0.75rem;
                    display: grid;
            ...
            {data.length > 0 && (
                <>
                    ...
                    >
                        ...
                        <input
                            type="text"
                            placeholder="Search by token ID (e.g. 9 or #9), RFID, hash…"
                            value={searchTerm}
                    ...
                    >
                        ...
                        collectibles (total: {data.length})
                    </div>
                    ...
                    {sortedData.length === 0 ? (
                        ...
                        <div className="collectibles-grid">
                            {pageItems.map((c) => {
                                ...
                                // Match AllCollectiblesPage behavior (use backend URLs if present)
                                const imageUrl = c.imageCardUrl ?? c.imageThumbUrl ?? null;

                                return (
                                    <div key={cardKey} className="collectible-card">
                                        {/* Thumbnail header with fixed aspect ratio (same formatting) */}
                                        <div
                                            style={{
                                                aspectRatio: "3 / 4",
                                                overflow: "hidden",
                                        ...
                                        >
                                            {/* Token row (to match the “All” page layout) */}
                                            <div
                                                style={{
                                                    display: "flex",
